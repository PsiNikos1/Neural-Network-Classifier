   ...
   public void setLeanringRate() {
    ...
    public void  backProp(Double[] x , int d , Double[] tn , Double[] on ) {//Returns the partials derivatives of En with respect to wi.
    	...
    	for(int n=0; n<this.outputNeuronsList.size(); n++) {
    		...
    		newDerivatives[0] = dList[n] * this.H2List.get(0).neuronOutput() ;
    		newDerivatives[1] = dList[n] * this.H2List.get(1).neuronOutput() ;
    		newDerivatives[2] = dList[n] * this.H2List.get(2).neuronOutput() ;
    		newDerivatives[3] = dList[n] * this.H2List.get(3).neuronOutput() ;
    		newDerivatives[4] = dList[n];
    	...
    	for(int n=0; n<this.H2List.size(); n++) {
    		...
    		this.H2List.get(n).setDelta(delta);
    	}
    	...
    	for(int n=0; n<this.H1List.size(); n++) {
    		...
    		this.H1List.get(n).setDelta(delta);
    	}
    ...
    public void trainWithGradientDescent() {
    	BigDecimal  previousError = BigDecimal.valueOf(10000.0);
    	...
    	for(int epoch = 0; epoch<1000; epoch++){
	    	...
	    	for(Double[] x :this.trainingDataSet) {
   	    	...
   	    	/*if(epoch>700 ) {
   	    		BigDecimal diff = previousError.subtract(epochError).divide(BigDecimal.valueOf(this.trainingDataSet.size())) ;
	    	    System.out.println("prev: "+previousError.divide(BigDecimal.valueOf(this.trainingDataSet.size())).toPlainString());

	    	    System.out.println("diff: "+diff.toPlainString());

   	    		if(previousError.compareTo(epochError) == 0 ) {
    ...
    public List<Double> getWeights(){
    	...
    	for(InputNeuron inp : this.inputNeuronsList){
    		for(Double w : inp.getWeights() ) {
    			list.add(w);
    		}
    	}
    ...
    public void createChart(List<Double[]> corrects , List<Double[]> incorrects) {
        ...
        String filename1 = "resources/correct(2HiddenLayers).png";
        String filename2 = "resources/incorrect(2HiddenLayers).png";
